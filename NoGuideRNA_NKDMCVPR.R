...
dCT_gRNA_NVPRCKDM$new_column <- c("PlexB", "Nlg2", "Lamp1", "Wun2", "Trxr2", "Dhc16")
colnames(dCT_gRNA_NVPRCKDM)[13] <- "gene_name"

df_dCT_GapDH <- dCT_gRNA_NVPRCKDM[,c(1:6, 13)]
df_dCT_Rps9 <- dCT_gRNA_NVPRCKDM[,c(7:13)]
...
barplot(bar_matrix,
        ...
        las = 2,
        ylab = "ΔCT",
        main = "Target Gene Expression for NVPRCKDM with GapDH as a Reference Gene")
...
barplot(bar_matrix_rps9,
        ...
        las = 2,
        ylab = "ΔCT",
        main = "Target Gene Expression for NVPRCKDM with Rps9 as a Reference Gene")
...
NVPRCKDM_sd <- cbind(df_GapDH_pS2$sd_pS2, df_GapDH_mS2$sd_mS2, df_Rps9_pS2$sd_pS2, df_Rps9_mS2$sd_mS2)
rownames(NVPRCKDM_sd) <- c("PlexB", "Nlg2", "Lamp1", "Wun2", "Trxr2", "Dhc16")
barplot(t(NVPRCKDM_sd),
        beside = TRUE,
        col = rep(c("cadetblue", "salmon", "skyblue", "coral")),  
        legend.text = c("GapDH +S2", "GapDH -S2", "Rps9 +S2", "Rps9 -S2"),
        ylim = range(as.vector(NVPRCKDM_sd), na.rm = TRUE) + c(-1, 1),
        args.legend = list(x = "topright", inset = c(0, -0.1), fill = c("cadetblue", "salmon", "skyblue", "coral")),
        names.arg = colnames(NVPRCKDM_sd),
        las = 2,
        ylab = "ΔCT",
        main = "NVPRCKDM Standard Deviation of ΔCT")
